...
export const createPlayerName = (user: UserData & RoomStatus) => {
  const { userId, userName, spaceName } = user;
  const {
    user: { userId: myUserId },
  } = useRendererStore.getState();
  return `
      <div id="${userId}_indicator" style="display: flex; align-items: center; justify-content:center; color: rgba(255, 255, 255, 0.8); font-size: 10px; font-family: Inter; font-weight: bold; ${
    myUserId === userId ? "background: rgba(0, 0, 0, 0.4)" : ""
  }; padding: 2px 9px; border-radius: 50px; width: 67px; overflow: hidden;">
